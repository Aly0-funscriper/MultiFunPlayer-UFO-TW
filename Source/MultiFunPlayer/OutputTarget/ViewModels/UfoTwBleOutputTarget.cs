...
[DisplayName("UFO-TW BLE (StableLink v3)")]
internal sealed class UfoTwBleOutputTarget : AsyncAbstractOutputTarget, IHandle<MediaPlayingChangedMessage>
{
    ...
    protected override async Task RunAsync(ConnectionType connectionType, CancellationToken token)
    {
        ...
        try
        {
            ...
            // Keep the physical GATT link alive. Without a session Windows can
            // release the ESP32 connection between service discovery and the
            // first characteristic write, which surfaces as Unreachable.
            session = await AwaitWinRtAsync(
                GattSession.FromDeviceIdAsync(device.BluetoothDeviceId),
                "open UFO-TW GATT session",
                BleOperationTimeout,
                token);
            if (session != null)
            {
                session.MaintainConnection = true;
                Logger.Info("UFO-TW GATT session opened [Status: {0}, MaintainConnection: {1}]",
                    session.SessionStatus, session.MaintainConnection);
            }

            await WaitForConnectionStabilizationAsync(device, token);
    ...
    private static async Task WaitForConnectionStabilizationAsync(BluetoothLEDevice device, CancellationToken token)
    {
        if (device == null)
            throw new OutputTargetException("Windows released the UFO-TW BLE device before writing");

        // A successful service query does not guarantee that WinRT has fully
        // transitioned the link to Connected. Give the ESP32 and Windows GATT
        // stack a short bounded window before the first write.
        ...
        if (device.ConnectionStatus != BluetoothConnectionStatus.Connected)
            throw new OutputTargetException("Windows did not establish the physical UFO-TW BLE connection");
        ...
        CancellationToken token)
    {
        ...
        try
        {
            ...
            throw new OutputTargetException("UFO-TW BLE packet failed after 4 attempts", lastError);
        }
        finally
        {
        ...
        for (var attempt = 1; attempt <= 4; attempt++)
        {
            ...
            try
            {
                ...
                var legacyOriginalCached = await AwaitWinRtAsync(
                    ...
                    "read cached legacy original UFO-TW service",
                    BleOperationTimeout,
                    token);
